...
/// The full coordinator role + workflow system prompt. Ported near-
/// verbatim from CC-fork's `getCoordinatorSystemPrompt()`. Tool names
/// (`Agent`, `SendMessage`, `TaskStop`) are the sudocode names — the
/// CC-fork uses the same tool names in its default (non-SIMPLE) branch.
///
