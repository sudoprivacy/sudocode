...
/// How long to wait for the *first byte* of piped stdin before giving up.
///
/// Matches Claude Code (`main.tsx` `getInputPrompt` -> `peekForStdinData`),
/// including its reasoning: 3s covers slow producers — curl, jq on a large
/// file, python paying import cost — while bounding the case that otherwise
/// hangs forever.
...
fn stdin_is_readable(timeout: Duration) -> bool {
    ...
    let timeout = PollTimeout::try_from(timeout).unwrap_or(PollTimeout::from(3_000u16));
...
/// when nothing arrives before the deadline, or when the piped content is
/// empty after trimming. Returns `Some(raw_content)` when a pipe delivered
/// non-empty content.
fn read_piped_stdin() -> Option<String> {
    if io::stdin().is_terminal() {
        return None;
    }
    read_piped_input(io::stdin(), STDIN_FIRST_BYTE_TIMEOUT, stdin_is_readable)
}

/// Read `reader` to EOF, but only once `is_readable` says something is there.
///
/// The deadline covers **only the first byte**. Once a producer has started
/// writing we wait as long as it needs, because that is the difference
/// between "this pipe is slow" (wait) and "nobody is on the other end"
/// (proceed). A total deadline would truncate `cat huge-file | scode -p ...`.
///
/// Checking readability first, rather than starting a read and abandoning it,
/// is the whole point: a blocking read on a pipe cannot be interrupted, and a
/// read left parked does not merely leak a thread — measured here, `--print`
/// against a pipe that stayed open served its deadline, printed its warning,
/// issued its API request, and then never saw the response, until the pipe
/// finally closed 75s later. With the read never started, the same command
/// finishes in 5s.
fn read_piped_input<R>(
    mut reader: R,
    first_byte_timeout: Duration,
    is_readable: impl Fn(Duration) -> bool,
) -> Option<String>
where
    R: Read,
{
    if !is_readable(first_byte_timeout) {
        // Warn rather than fail: the run is still valid without the piped
        // context, but silent data loss would not be.
        eprintln!(
            "warning: no stdin data received in {}s, proceeding without it. \
             If you are piping from a slow command, redirect stdin explicitly \
             (`< /dev/null` to skip it).",
