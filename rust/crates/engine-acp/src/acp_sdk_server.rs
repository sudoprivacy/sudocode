...
impl AcpError {
    ...
    pub fn failure_class(&self) -> &'static str {
        match self {
            ...
            Self::Turn(error) => error.failure_class(),
            Self::Internal(message) if message.contains(runtime::COMPACTION_FAILED) => {
                "compaction_failed"
            }
            Self::Internal(_) => "model_error",
        }
    }

    /// The same safe summary is sent as prose and as the RPC message. Diagnostic
    /// text belongs in RPC data; no unknown error may become an assistant answer.
    #[must_use]
    pub fn user_friendly_message(&self) -> String {
        match self {
            Self::InvalidParams(_) => "请求内容不符合要求，请检查输入后再试。".to_string(),
            Self::Turn(error) => error.user_message().to_string(),
            Self::Internal(_) if self.failure_class() == "compaction_failed" => {
                COMPACTION_FAILED_MESSAGE.to_string()
            }
            Self::Internal(_) => {
                "本次操作未能完成，请检查当前任务状态；若问题持续，请联系管理员。".to_string()
            }
        }
    }
}
...
pub(crate) fn acp_error_to_sdk(e: &AcpError) -> Error {
    let mut error = match e {
        AcpError::InvalidParams(_) => Error::invalid_params(),
        AcpError::Internal(_) | AcpError::Turn(_) => Error::internal_error(),
    };
    error.message = e.user_friendly_message();
    error.data = Some(json!({
        "error_type": e.failure_class(),
