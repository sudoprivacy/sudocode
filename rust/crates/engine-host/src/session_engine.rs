...
impl engine_core::EngineDelegate for SessionEngine {
    fn run_turn(
    ...
    ) -> Result<engine_core::TurnComplete, engine_core::TurnError> {
        let mut session = self.lock_session();
        session.abort_signal.reset();
        let _scope = runtime::WorkspaceRootScope::enter(&session.cwd);
        ...
        if estimated_tokens + prompt_tokens > history_budget {
            ...
            pre_send_compaction = attempt.map_err(|error| engine_core::TurnError {
            ...
            if !budget.fits(new_estimated_tokens + prompt_tokens) {
                return Err(engine_core::TurnError {
                    message: context_overflow_user_message(
                        session.runtime.session(),
        ...
        let turn_summary = self
            ...
            .map_err(engine_core::TurnError::from)?;
        ...
        session
            ...
            .session()
            .save_to_path(&path)
            .map_err(|e| engine_core::TurnError {
                message: format!("failed to persist session: {e}"),
                error_type: "runtime_io",
            })?;
